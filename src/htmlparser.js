...
const { createMapFromString } = require('./utils.js');
...
function joinSingleAttrAssigns(handler) {
  return singleAttrAssigns.concat(handler.customAttrAssign || [])
    .map(assign => '(?:' + assign.source + ')')
    .join('|');
}
...
function HTMLParser(html, handler) {
  ...
  while (html) {
    ...
    if (!lastTag || !special(lastTag)) {
      ...
      if (nextTagMatch) {
        ...
        nextTag = nextTagMatch ? '/' + nextTagMatch[1] : '';
      }
  ...
  function handleStartTag(match) {
    ...
    if (handler.html5) {
      if (lastTag === 'p' && nonPhrasing(tagName)) {
        ...
        closeIfFound('thead');
      } else if (tagName === 'tfoot' && !closeIfFound('tbody')) {
        closeIfFound('thead');
      }
  ...
  function parseEndTag(tag, tagName) {
    ...
    const pos = tagName ? findTag(tagName) : 0;
  ...
  new HTMLParser(html, {
    start(tagName, attrs, unary) {
      ...
      if (structure[tagName] && typeof one[structure[tagName]] !== 'boolean') {
        one[structure[tagName]].append(elem);
      } else if (curParentNode && curParentNode.appendChild) {
        curParentNode.append(elem);
      }
    ...
    chars(text) {
      curParentNode.append(doc.createTextNode(text));
    },
    comment(/* text */) {
      // create comment node
    },
